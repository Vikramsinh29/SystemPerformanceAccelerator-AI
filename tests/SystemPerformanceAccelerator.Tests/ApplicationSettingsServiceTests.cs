...
public sealed class ApplicationSettingsServiceTests
{
    ...
    public void Load_WhenFileDoesNotExist_ReturnsSafeDefaults()
    {
        ...
        var service = new ApplicationSettingsService(location.SettingsPath);
    ...
    {
        using var location = new TemporarySettingsLocation();
        Directory.CreateDirectory(Path.GetDirectoryName(location.SettingsPath)!);
        File.WriteAllText(location.SettingsPath, "{ invalid json");
        var service = new ApplicationSettingsService(location.SettingsPath);
    ...
    public void Load_WhenValuesAreUnsafe_NormalizesThemAndKeepsConfirmationEnabled()
    {
        ...
        Assert.Equal(ApplicationTheme.Light, result.Settings.Theme);
        ...
        Assert.Equal(
            ...
            result.Settings.SystemMonitorRefreshIntervalSeconds);
        Assert.True(result.HasWarning);
    }

    private sealed class TemporarySettingsLocation : IDisposable
    {
